...
type AuthOptions struct {
	AuthType     string
	LogicType    LogicType
	AuthFailFunc func(c echo.Context, err error) error
}
...
func WithAuthFailFunc(fn func(c echo.Context, err error) error) AuthOption {
	return func(o *AuthOptions) {
		o.AuthFailFunc = fn
	}
}
...
func AuthMiddleware(opts ...AuthOption) echo.MiddlewareFunc {
...
func AuthWithStateMiddleware(opts ...AuthOption) echo.MiddlewareFunc {
	options := defaultAuthOptions()
	for _, opt := range opts {
		opt(options)
	}
...
// GetSaTokenContext gets Sa-Token context from Echo context | 获取 Sa-Token 上下文
func GetSaTokenContext(c echo.Context) (*saContext.SaTokenContext, bool) {
	v := c.Get(SaTokenCtxKey)
	if v == nil {
		return nil, false
	}
...
func getSaContext(c echo.Context, mgr *manager.Manager) *saContext.SaTokenContext {
	// Try get from context | 尝试从 ctx 取值
	if v := c.Get(SaTokenCtxKey); v != nil {
	...
	saCtx := saContext.NewContext(NewEchoContext(c), mgr)
